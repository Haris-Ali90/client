...
class CtcEntriesController extends BackendController
{
    ...
    public function getCtcDashboardBrokerData(Datatables $datatables, Request $request)
    {
		...
		$postal_code = SlotPostalCode::whereIn('zone_id',$microHubZone)->pluck('postal_code')->toArray();
        ...
        if (!empty($request->get('tracking_id'))) {
            $query = CTCEntry::join('sprint__tasks', 'sprint__tasks.id', '=', 'ctc_entries.task_id')
                ->join('locations', 'locations.id', '=', 'sprint__tasks.location_id')
                ->where('ctc_entries.sprint_id', $sprintId)
                ->where('ctc_entries.deleted_at', null)
            ...
            $query = CTCEntry::join('sprint__tasks', 'sprint__tasks.id', '=', 'ctc_entries.task_id')
                ->join('locations', 'locations.id', '=', 'sprint__tasks.location_id')
                ->whereIn('ctc_entries.sprint_id', $sprintIds)
                ->where('ctc_entries.deleted_at', null)
                ->whereIn(\DB::raw('SUBSTRING(locations.postal_code,1,3)'),$postal_code)
                ->distinct()
                ->select(['ctc_entries.*'
            ...
            $query = CTCEntry::join('sprint__tasks', 'sprint__tasks.id', '=', 'ctc_entries.task_id')
                ->join('locations', 'locations.id', '=', 'sprint__tasks.location_id')
                ->whereIn('ctc_entries.creator_id', $ctcVendorIds)->where('ctc_entries.created_at','>',$start)->where('ctc_entries.created_at','<',$end)
                ->whereNotIn('ctc_entries.task_status_id', [38, 36])
                ->whereIn(\DB::raw('SUBSTRING(locations.postal_code,1,3)'),$postal_code)
                ->distinct()
                ->select(['ctc_entries.*'
                ]);
    ...
    public function getCtcDashboardData(Datatables $datatables, Request $request)
    {
        ...
        if (!empty($request->get('tracking_id'))) {
            ...
            $query = CTCEntry::join('sprint__tasks', 'sprint__tasks.id', '=', 'ctc_entries.task_id')
                ->join('locations', 'locations.id', '=', 'sprint__tasks.location_id')
                ->whereIn(\DB::raw('SUBSTRING(locations.postal_code,1,3)'),$postal_code)
                ->whereIn('ctc_entries.creator_id', $ctcVendorIds)
    ...
    public function ctcDashboardExcel($date = null)
    {
        ...
        $ctc_data = CTCEntry::join('sprint__tasks', 'sprint__tasks.id', '=', 'ctc_entries.task_id')
            ->join('locations', 'locations.id', '=', 'sprint__tasks.location_id')
            ->whereIn('ctc_entries.creator_id', $ctcVendorIds)
            ->where('ctc_entries.updated_at','>',$start)
            ->where('ctc_entries.updated_at','<',$end)
            ->whereNotIn('ctc_entries.task_status_id', [38, 36])
            ->whereIn(\DB::raw('SUBSTRING(locations.postal_code,1,3)'),$postal_code)
            ->distinct()
            ->get();
...
//            echo ($actual_delivery_status == 13) ? "At hub - processing"."\t" : self::$status[$actual_delivery_status] . "\t";
    ...
    public function ctcDashboardExcelOtdReport($date = null)
    {
        ...
        $ctc_data = CTCEntry::join('sprint__tasks', 'sprint__tasks.id', '=', 'ctc_entries.task_id')
            ->join('locations', 'locations.id', '=', 'sprint__tasks.location_id')
            ->whereIn('ctc_entries.creator_id', $ctcVendorIds)
            ->whereIn('ctc_entries.sprint_id', $sprint_id)
            ->whereNotIn('ctc_entries.task_status_id', [38, 36])
            ->whereIn(\DB::raw('SUBSTRING(locations.postal_code,1,3)'),$postal_code)
            ->distinct()
            ->get();
    ...
    public function ctcTotalCards($date, $type)
    {
        ...
        $taskIds = DB::table('ctc_entries')->join('sprint__tasks', 'sprint__tasks.id', '=', 'ctc_entries.task_id')
            ->join('locations', 'locations.id', '=', 'sprint__tasks.location_id')
            ->whereIn('ctc_entries.creator_id', $ctcVendorIds)
            ->where('ctc_entries.created_at','>',$start)
            ->where('ctc_entries.created_at','<',$end)
            ->where('ctc_entries.is_custom_route', 0)
            ->whereNotIn('ctc_entries.task_status_id', [38, 36])
            ->whereIn(\DB::raw('SUBSTRING(locations.postal_code,1,3)'),$postal_code)
            ->distinct()
            ->pluck('ctc_entries.task_id');
    ...
    public function getCtcYesterdayOrderData($date)
    {
        ...
        $yesterday_return_orders = DB::table('ctc_entries')
            ->whereIn('creator_id', $ctcVendorIds)
            ->join('sprint__tasks', 'sprint__tasks.id', '=', 'ctc_entries.task_id')
            ->join('locations', 'locations.id', '=', 'sprint__tasks.location_id')
            ->join('sprint_reattempts', 'ctc_entries.sprint_id', '=', 'sprint_reattempts.sprint_id')
            ->where('ctc_entries.created_at','>',$start)
            ->where('ctc_entries.created_at','<',$end)
            ->whereIn(\DB::raw('SUBSTRING(locations.postal_code,1,3)'),$postal_code)
            ->count();
    ...
    public function getCtcCustomRouteData($date)
    {
        ...
        $ctcVendorIds = CtcVendor::pluck('vendor_id');
        $custom_route = DB::table('ctc_entries')->join('sprint__tasks', 'sprint__tasks.id', '=', 'ctc_entries.task_id')
            ->join('locations', 'locations.id', '=', 'sprint__tasks.location_id')
            ->whereIn('ctc_entries.creator_id', $ctcVendorIds)
            ->where('ctc_entries.created_at','>',$start)
            ->where('ctc_entries.created_at','<',$end)
    ...
    public function ctcSortedData(Datatables $datatables, Request $request)
    {
        ...
        $query = CTCEntry::join('sprint__tasks', 'sprint__tasks.id', '=', 'ctc_entries.task_id')
            ...
            ->where('ctc_entries.is_custom_route', 0)
            ->whereNotIn('ctc_entries.task_status_id', [38, 36])
            ->where(['ctc_entries.task_status_id' => 133])
            ->whereIn(\DB::raw('SUBSTRING(locations.postal_code,1,3)'),$postal_code)
            ->distinct()
            ->select(['ctc_entries.*'
            ]);
    ...
    public function ctcPickedupExcel($date = null)
    {
        ...
        $ctc_data = CTCEntry::join('sprint__tasks', 'sprint__tasks.id', '=', 'ctc_entries.task_id')
            ...
            ->where('ctc_entries.is_custom_route', 0)
            ->whereIn('ctc_entries.creator_id', $ctcVendorIds)
            ->where(['ctc_entries.task_status_id' => 121])
            ->whereNotIn('ctc_entries.task_status_id', [38, 36])
            ->whereIn(\DB::raw('SUBSTRING(locations.postal_code,1,3)'),$postal_code)
            ->get();
        $ctc_array[] = ['JoeyCo Order #', 'Route Number', 'Joey', 'Customer Address', 'Out For Delivery', 'Sorted Time', 'Actual Arrival @ CX', 'CTC tracking #', 'Status'];
    ...
    public function ctcNotScanData(Datatables $datatables, Request $request)
    {
        ...
        $query = CTCEntry::join('sprint__tasks', 'sprint__tasks.id', '=', 'ctc_entries.task_id')
            ...
            ->whereIn('ctc_entries.task_status_id', [61, 13])
            ->whereIn(\DB::raw('SUBSTRING(locations.postal_code,1,3)'),$postal_code)
            ->distinct()
            ->select(['ctc_entries.*'
            ]);
    ...
    public function ctcDeliveredData(Datatables $datatables, Request $request)
    {
        ...
        $query = CTCEntry::join('sprint__tasks', 'sprint__tasks.id', '=', 'ctc_entries.task_id')
            ...
            ->where('ctc_entries.is_custom_route', 0)
            ->whereNotIn('ctc_entries.task_status_id', [38, 36])
            ->whereIn('ctc_entries.task_status_id', [17, 113, 114, 116, 117, 118, 132, 138, 139, 144])
            ->whereIn(\DB::raw('SUBSTRING(locations.postal_code,1,3)'),$postal_code)
            ->distinct()
            ->select(['ctc_entries.*'
            ]);
    ...
    public function ctcReturnedData(Datatables $datatables, Request $request)
    {
        ...
        $query = CTCEntry::join('sprint__tasks', 'sprint__tasks.id', '=', 'ctc_entries.task_id')
            ...
            ->whereIn('ctc_entries.creator_id', $ctcVendorIds)
            ->whereNotIn('ctc_entries.task_status_id', [38, 36])
            ->whereIn('ctc_entries.task_status_id', [101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 131, 135, 136, 143])
            ->where('ctc_entries.is_custom_route', 0)
            ->whereIn(\DB::raw('SUBSTRING(locations.postal_code,1,3)'),$postal_code)
            ->distinct()
            ->select(['ctc_entries.*'
    ...
    public function ctcNotReturnedData(Datatables $datatables, Request $request)
    {
        ...
        $query = CTCEntry::join('sprint__tasks', 'sprint__tasks.id', '=', 'ctc_entries.task_id')
            ...
            ->whereIn('ctc_entries.task_status_id', [101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 131, 135, 136, 143])
            ->where('ctc_entries.is_custom_route', 0)
            ->whereNull('ctc_entries.hub_return_scan')
            ->whereIn(\DB::raw('SUBSTRING(locations.postal_code,1,3)'),$postal_code)
            ->distinct()
            ->select(['ctc_entries.*'
            ]);
    ...
    public function getRouteinfo(Request $request)
    {
        ...
        return backend_view('ctc-entries.ctc_route_info', compact('ctc_info','flagCategories','ctcVendorIds'));
    }
    ...
    public function getCtcReporting(Request $request)
    {
        ...
        $sprint_ids = CTCEntry::join('sprint__tasks', 'sprint__tasks.id', '=', 'ctc_entries.task_id')
            ->join('locations', 'locations.id', '=', 'sprint__tasks.location_id')
            ->whereIn('ctc_entries.creator_id', $ctcVendorIds)
            ->where('ctc_entries.created_at','>',$start)
            ->where('ctc_entries.created_at','<',$end)
    ...
    public function ajax_render_ctc_otd_day(Request $request)
    {
        ...
        $query = CTCEntry::join('sprint__tasks', 'sprint__tasks.id', '=', 'ctc_entries.task_id')
            ...
            ->where('ctc_entries.created_at','<',$end)
            ->whereNotIn('ctc_entries.task_status_id', [38, 36])
            ->whereIn('ctc_entries.task_status_id', $sprint->getStatusCodes('competed'))
            ->whereIn(\DB::raw('SUBSTRING(locations.postal_code,1,3)'),$postal_code)
            ->distinct()
            ->select(['ctc_entries.*'
            ]);
    ...
    public function ajax_render_ctc_otd_week(Request $request)
    {
        ...
        $query = CTCEntry::join('sprint__tasks', 'sprint__tasks.id', '=', 'ctc_entries.task_id')
            ...
            ->whereIn('ctc_entries.task_status_id', $sprint->getStatusCodes('competed'))
            ->whereNotIn('ctc_entries.task_status_id', [38, 36])
            ->whereBetween(\DB::raw("CONVERT_TZ(ctc_entries.created_at,'UTC','America/Toronto')"), [date('y-m-d', strtotime('-6 day', strtotime($date))) . ' 20:00:00', $date . " 19:59:59"])
            ->whereIn(\DB::raw('SUBSTRING(locations.postal_code,1,3)'),$postal_code)
            ->distinct()
            ->select(['ctc_entries.*'
            ]);
    ...
    public function ajax_render_ctc_otd_month(Request $request)
    {
        ...
        $query = CTCEntry::join('sprint__tasks', 'sprint__tasks.id', '=', 'ctc_entries.task_id')
            ...
            ->whereIn('ctc_entries.task_status_id', $sprint->getStatusCodes('competed'))
            ->whereNotIn('ctc_entries.task_status_id', [38, 36])
            ->whereBetween(\DB::raw("CONVERT_TZ(ctc_entries.created_at,'UTC','America/Toronto')"), [date('y-m-d', strtotime('-1 month', strtotime($date))) . ' 20:00:00', $date . " 19:59:59"])
            ->whereIn(\DB::raw('SUBSTRING(locations.postal_code,1,3)'),$postal_code)
            ->distinct()
            ->select(['ctc_entries.*'
            ]);
