...
class FirstMileReportingController extends BackendController
{
    ...
    public function getFirstMileReporting(Request $request)
    {
        ...
        $first_mile_data = CTCEntry::whereIn('creator_id', $ctcVendorIds)
    ...
    public function getFirstMileReportingData(Datatables $datatables, Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            {
                $query = CTCEntry::whereIn('creator_id', $ctcVendorIds)
                    ->whereIn('sprint_id', $sprint_id_data1)
                    ->whereBetween('created_at', [$from_date, $to_date])
                    ->whereNotIn('task_status_id', [38, 36])
                    ->where('task_status_id', 61)
                    ->groupBy('sprint_id')
                    ->select(['ctc_entries.*','task_status_id as tasks_history_status_id']);
            }
            else
            {
                $query = CTCEntry:: join('sprint__tasks_history', 'sprint__tasks_history.sprint_id', '=', 'ctc_entries.sprint_id')
                    ->whereIn('ctc_entries.creator_id', $ctcVendorIds)
                    ->whereIn('ctc_entries.sprint_id', $sprint_id_data1)
                    ->whereBetween('ctc_entries.created_at', [$from_date, $to_date])
                    ->whereNotIn('ctc_entries.task_status_id', [38, 36])
                    ->groupBy('sprint__tasks_history.sprint_id','ctc_entries.sprint_id')
                    ->select(['ctc_entries.*','sprint__tasks_history.status_id as tasks_history_status_id']);
            }
            //$query = CTCEntry:: join('sprint__tasks_history', 'sprint__tasks_history.sprint_id', '=', 'ctc_entries.sprint_id')
            //    ->whereIn('ctc_entries.creator_id', $ctcVendorIds)
            //    ->whereIn('ctc_entries.sprint_id', $sprint_id_data1)
            ...
            return $datatables->eloquent($query)
                ...
                ->addColumn('picked_up_at', static function ($record) {
                    if ($record->pickedAtStore) {
                        return $record->pickedAtStore->date;
                    } else {
                        "";
                    }
                })
                ->addColumn('delivered_at', static function ($record) {
                    if ($record->deliveredAtHub) {
                        return $record->deliveredAtHub->date;
                    } else {
                        "";
                    }
                })
