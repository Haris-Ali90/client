...
class VendorManagementController extends BackendController
{
    ...
    public function getDayOtd(Request $request)
    {
        ...
        $userid = Auth::user()->id;
        //isset($input['vendor_id']) ?  $input['vendor_id'] : 477661;
//        $vendors = Vendor::where('id', $vendor_id)->pluck('id')->toArray();
//        $borderLessVendorIds = BoradlessDashboard::pluck('creator_id')->toArray();
        $sprint = new Sprint();

        $totalcount = 0;
        $totallates = 0;
        $totalerly = 0;
//        $amazon_date = date('Y-m-d', strtotime($start_date . ' -1 days'));
        ...
        $query = BoradlessDashboard::where('creator_id', $userid)->where('created_at','>',$start)->where('created_at','<',$end)->whereNotNull('delivered_at')
...
//                if (in_array($record->task_status_id,$sprint->getStatusCodes('competed'))) {
                if (!is_null($record->delivered_at) && $record->delivered_at > $start_date . " 21:00:00") {
                    $totallates++;
                }
//                }
//                if (in_array($record->task_status_id,$sprint->getStatusCodes('return'))) {
                if (!is_null($record->returned_at)  && $record->delivered_at < $start_date . " 21:00:00") {
                    $totalerly++;
                }
//                }
                $totalcount++;
...
//        if (count(array_intersect($borderLessVendorIds, $vendors))> 0) {
//            $ctc_ids = array_intersect($borderLessVendorIds, $vendors);
...
//        }
        ...
        $odt_data_1 = ['y1' => round(($totalerly / $totalcount) * 100, 2), 'y2' => round(100 - (($totallates / $totalcount) * 100), 2) ,'ontime'=>  $totallates  , 'offtime'=> $totalerly ];
    ...
    public function getWeekOtd(Request $request)
    {
        ...
        $userid = Auth::user()->id;
        //isset($input['vendor_id']) ?  $input['vendor_id'] : 477661;
        $vendors = Vendor::where('id', $userid)->pluck('id')->toArray();
...
//            if (in_array('477661', $vendors)) {
            ...
            $query = BoradlessDashboard::where('creator_id', $userid)->where('created_at','>',$start)->where('created_at','<',$end)
                ->where('is_custom_route', 0)->get(['task_status_id', \DB::raw("CONVERT_TZ(delivered_at,'UTC','America/Toronto') as delivered_at"),
                    \DB::raw("CONVERT_TZ(returned_at,'UTC','America/Toronto') as returned_at")]);
            if (!empty($query)) {
                foreach ($query as $record) {
...
//            if (in_array('477661', $vendors)) {
            ...
            $query = BoradlessDashboard::where('creator_id', $vendor_id)->where('created_at','>',$start)->where('created_at','<',$end)
                ->where('is_custom_route', 0)->get(['task_status_id', \DB::raw("CONVERT_TZ(delivered_at,'UTC','America/Toronto') as delivered_at"),
                    \DB::raw("CONVERT_TZ(returned_at,'UTC','America/Toronto') as returned_at")]);
            if (!empty($query)) {
...
//            $amazon_date = date('Y-m-d', strtotime($date . ' -1 days'));
        ...
        $taskIds = DB::table('boradless_dashboard')->where('created_at','>',$start)->where('created_at','<',$end)
            ->where(['creator_id' => $vendor_id])->where('is_custom_route', 0)->pluck('task_id');
        $boradlessDashboard = new BoradlessDashboard();
        $boradlessDashboardCounts = $boradlessDashboard->getBoradlessCountsForLoop($taskIds, 'all');
//        }
...
//        }
    ...
    public function getOrders(Request $request)
    {
        ...
        if (in_array('$vendor_id', $vendors)) {
            ...
            if ($type == 'return') {
                $montreal = DB::table('amazon_enteries')->where('created_at','>',$start)->where('created_at','<',$end)
                    ->where(['creator_id' => $vendors])->where('is_custom_route', 0)->whereIn('task_status_id', $sprint->getStatusCodes('return'))->get();
            }
            if ($type == 'hub_return') {
                $montreal = DB::table('amazon_enteries')->where('created_at','>',$start)->where('created_at','<',$end)
                    ->where(['creator_id' => $vendors])->where('is_custom_route', 0)->whereIn('task_status_id', $sprint->getStatusCodes('return'))
            ...
            if ($type == 'not_scan'){
                $montreal = DB::table('amazon_enteries')->where('created_at','>',$start)->where('created_at','<',$end)
                    ->where(['creator_id' => $vendors])->where('is_custom_route', 0)->whereIn('task_status_id',[61])
                    ->whereNull('hub_return_scan')->get();
            }
            if ($type == 'reattempted'){
                $montreal = DB::table('amazon_enteries')->where('created_at','>',$start)->where('created_at','<',$end)
                    ->where(['creator_id' => $vendors])->where('is_custom_route', 0)->whereIn('task_status_id',[13])
                    ->whereNull('hub_return_scan')->get();
            }
            ...
            if ($type == 'custom'){
                ...
                $montreal = DB::table('amazon_enteries')->where('created_at','>',$start)->where('created_at','<',$end)
                    ->where(['creator_id' => $vendors])->where('is_custom_route', 1)
                    ->whereNull('hub_return_scan')->get();
            }
        }
        ...
        if (in_array('$vendor_id', $vendors)) {
            ...
            if ($type == 'reattempted'){
                $ottawa = DB::table('amazon_enteries')->where('created_at','>',$start)->where('created_at','<',$end)
                    ->where(['creator_id' => $vendor_id])->where('is_custom_route', 0)->whereIn('task_status_id',[13])
                    ->whereNull('hub_return_scan')->get();
            }
            if ($type == 'custom') {
                $start_dt = new DateTime($date." 00:00:00", new DateTimezone('America/Toronto'));
                ...
                $ottawa = DB::table('amazon_enteries')->where('created_at','>',$start)->where('created_at','<',$end)
                    ->where(['creator_id' => $vendor_id])->where('is_custom_route', 1)
                    ->whereNull('hub_return_scan')->get();
            }
        }
    ...
    public function getFailedOrders(Request $request)
    {
        ...
        if (in_array('$vendor_id', $vendors)) {
            ...
            $failedTrackingId = DB::table('xml_failed_orders')->join('mainfest_fields', 'mainfest_fields.trackingID', '=', 'xml_failed_orders.tracking_id')
                ...
                ->whereNull('mainfest_fields.deleted_at')->where(['vendor_id' => $vendors])
                ->pluck('trackingID');
        ...
        if (in_array('$vendor_id', $vendors)) {
            ...
            $failedTrackingId = DB::table('xml_failed_orders')->join('mainfest_fields', 'mainfest_fields.trackingID', '=', 'xml_failed_orders.tracking_id')
                ...
                ->whereNull('mainfest_fields.deleted_at')->where(['vendor_id' => $vendors])->pluck('trackingID');
            if ($type == 'failed') {
    ...
    public function getFailedCounts(Request $request)
    {
        $input = $request->all();
        $date = $request->get('start_date') ? $request->get('start_date') : date('Y-m-d');
        $vendor_id = isset($input['vendor_id']) ?  $input['vendor_id'] : vendor_id;
        $vendors = Vendor::where('id', $vendor_id)->pluck('id')->toArray();
...
//dd($vendors);
        if (in_array('vendor_id', $vendors)) {
            ...
            $failed_tracking_Ids = DB::table('xml_failed_orders')->join('mainfest_fields', 'mainfest_fields.trackingID', '=', 'xml_failed_orders.tracking_id')
                ->where('xml_failed_orders.created_at','>',$start)->where('xml_failed_orders.created_at','<',$end)->whereNotNull('mainfest_fields.trackingID')
                ->whereNull('mainfest_fields.deleted_at')->where(['vendor_id' => $vendor_id])->pluck('tracking_id');
    ...
    public function getCustomCounts(Request $request)
    {
        ...
        if (in_array('$vendor_id', $vendors)) {
            ...
            $custom_order = $custom_order + DB::table('amazon_enteries')->where('created_at','>',$start)->where('created_at','<',$end)
                    ->where(['creator_id' => $vendors])->where('is_custom_route', 1)->count();
        }
//        if (in_array('477282', $vendors)) {
//
//            $start_dt = new DateTime($date." 00:00:00", new DateTimezone('America/Toronto'));
    ...
    public function getManualCounts(Request $request)
    {
        ...
        if (in_array($vendor_id, $vendors)) {
            ...
            $trackingIds = DB::table('amazon_enteries')->where('created_at','>',$start)->where('created_at','<',$end)
                ->where(['creator_id' => vendor_id])->pluck('tracking_id');
    ...
    public function getTopTenJoeys(Request $request)
    {
        ...
        if (in_array($vendor_id, $vendors)) {
            ...
            $query = DB::table('amazon_enteries')->where('created_at','>',$start)->where('created_at','<',$end)
                ->where(['creator_id' => $vendor_id])->get(['tracking_id','joey_id','task_status_id', \DB::raw("CONVERT_TZ(delivered_at,'UTC','America/Toronto') as delivered_at"),
                    \DB::raw("CONVERT_TZ(returned_at,'UTC','America/Toronto') as returned_at")]);
            if (!empty($query)) {
                foreach ($query as $record) {
                    if (in_array($record->task_status_id,$sprint->getStatusCodes('competed'))) {
