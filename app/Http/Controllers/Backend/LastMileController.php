...
class LastMileController extends Controller
{
    ...
    public function lastMileZoneUpdate(Request $request)
    {
        ...
        SlotsPostalCode::where('zone_id', '=', $request->input('id_time'))->delete();
    ...
    public function lastMileZoneDelete(Request $request)
    {
        ...
        return redirect()->back()->with('success', 'Micro Hub Zone Deleted Successfully!');
    }

    public function lastMileZoneOrderCount($date, $zone_id)
    {

        $hubStores = HubStore::where('hub_id', auth()->user()->hub_id)->whereNull('deleted_at')->pluck('vendor_id');
        ...
        $totalCounts = $this->ctcCount($vendors, $SlotsPostalCode);
    ...
    public function ctcCount($vendor,$postals=array()){
        ...
        foreach($postals as $postalcode){
          ...
          AND locations.postal_code like '".$postalcode->postal_code."%'
          AND sprint__sprints.status_id IN(13,124)
          AND date(sprint__sprints.created_at) > '2021-07-31'";
