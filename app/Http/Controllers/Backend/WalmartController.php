...
public function generate_walmart_report_csv(WalmartRequest $request)
    ...
    public function csv_uploader(Request $request){
        ...
        for ($i = 1; $i < count($request->all()['rowData']); $i++) {
            $sprintData = [
            ...
            // dd($sprint);
            $sprint_id = $sprint->id;
            $contact_data_pickup['name'] = $request->all()['rowData'][$i][1];
            $contact_data_pickup['phone'] = $request->all()['rowData'][$i][2];
            $contact_data_pickup['email'] = $request->all()['rowData'][$i][3];
            $contact_pickup = SprintContact::create($contact_data_pickup);
            // $sprint_history= SprintSprintHistory::insert(['sprint__sprints_id'=>$sprint_id,'vehicle_id'=>$sprint->vehicle_id,'distance'=>$sprint['distance'],
            ...
            $sprintTaskData=[
                ...
                'type'=>'pickup',
                'status_id'=>38,
                'contact_id'=>auth()->user()->id,//$contact_id_pickup
                'location_id'=>'test',//$location_id_pickup
                'active'=>1,
            ...
            $sprint_task_pickup=Task::create($sprintTaskData);
            $sprint_task_drop_off=Task::create($sprintTaskDatadropoff);
            ...
            SprintTaskHistory::insert(['created_at'=>date('Y-m-d H:i:s'),'date'=>date('Y-m-d H:i:s'),'sprint__tasks_id'=>$sprint_task_drop_off,'sprint_id'=>$sprint_id,'status_id'=>38,'active'=>1]);
            $ordinal++;
            ...
            $sprint_confirmation_drop_off['task_id']=$sprint_task_drop_off->id;
            $sprint_confirmation_drop_off['name']="default";
            $sprint_confirmation_drop_off['title']="Confirm Dropoff";
            SprintConfirmation::create($sprint_confirmation_default_data);
            SprintConfirmation::create($sprint_confirmation_drop_off);
            $csv = new BoradlessDashboard();
            $csv->joey_id = $request->all()['rowData'][$i][0];
            $csv->creator_id = auth()->user()->id;
            $csv->task_status_id =61;
            $csv->customer_name = $request->all()['rowData'][$i][1];
            $csv->address_line_1 = $request->all()['rowData'][$i][4];
            $csv->tracking_id = $request->all()['rowData'][$i][11];
            $csv->save();
            $merchant = new MerchantIds();
            $merchant->merchant_order_num = $request->all()['rowData'][$i][10];
            $merchant->start_time = $request->all()['rowData'][$i][12];
            $merchant->end_time = $request->all()['rowData'][$i][13];
            $merchant->tracking_id = $request->all()['rowData'][$i][11];
            $merchant->address_line2 = $request->all()['rowData'][$i][4];
            $merchant->package_count = $request->all()['rowData'][$i][8];
            $merchant->item_count = $request->all()['rowData'][$i][9];
            $merchant->save();
        }
        //   465248
        return back();
