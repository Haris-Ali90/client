...
class NewMontrealController extends BackendController
{
    ...
    public function getCustomRouteData($date)
    {
        ...
        $custom_route = DB::table('boradless_dashboard')->where('created_at', '>', $start)->where('created_at', '<', $end)
            ->whereIn('creator_id', $user->id)->where('is_cusktom_route', 1)->count();
    ...
    public function getMontreal(Datatables $datatables, Request $request)
    {
        //User Data to get the logged in details...
        $user = Auth::user();
        //Getting vendors_id from HubStore on the bases of hub_id...
        ...
        // 
        $user = Auth::user();
        if (request()->path() == 'date/reporting') {
            ...
            $end_dt->setTimeZone(new DateTimezone('UTC'));
            $end = $end_dt->format('Y-m-d H:i:s');
            $query = BoradlessDashboard::where('is_custom_route', 0)->where('created_at', '>', $start)->where('created_at', '<', $end)->where('creator_id', $user->id)->paginate(10);
        } else {
            $query = BoradlessDashboard::where('is_custom_route', 0)->where('creator_id', $user->id)->paginate(10);
        }
        $status = new BoradlessDashboard();
        $status_code = $status->statusCode();


        return backend_view('newmontrealdashboard.montreal_dashboard', compact('type', 'query', 'status_code'));
    }
    ...
    public function montrealData(Datatables $datatables, Request $request)
    {
        ...
        $query = BoradlessDashboard::where('created_at', '>', $start)->where('created_at', '<', $end)
            ->where('creator_id', $user->id)->where('is_custom_route', 0);
    ...
    public function montrealProfile(Request $request, $id)
    {
        ...
        $amazon_montreal = BoradlessDashboard::where(['id' => $mont_id])->get();
        $data = $this->get_trackingorderdetails($amazon_montreal[0]->sprint_id);
        $sprintId = $data['sprintId'];
        $data = $data['data'];
        return backend_view('newmontrealdashboard.montreal_profile', compact('data', 'sprintId', 'amazon_montreal'));
    }
    ...
    public function montrealExcel($date = null)
    {
        ...
        $montreal_data = BoradlessDashboard::where('created_at', '>', $start)->where('created_at', '<', $end)->where('is_custom_route', 0)->whereIn('creator_id', $data)->get();
    ...
    {
        $today_time = \Carbon\Carbon::now(); //$notification_query = BoradlessDashboard::where('is_custom_route', 0)->where('delivered_at','>=',now())
        //->get();
        $title_name = 'Montreal';
        $type = 'sorted';
        ...
        if ($request->get('datepicker')) {
            ...
            $end_dt->setTimeZone(new DateTimezone('UTC'));
            $end = $end_dt->format('Y-m-d H:i:s');
            $query = BoradlessDashboard::where('is_custom_route', 0)->where(['task_status_id' => $task_id])->where('created_at', '>', $start)->where('created_at', '<', $end)->where('creator_id', $user->id)->paginate(10);
        } else {
            $query = BoradlessDashboard::where('is_custom_route', 0)->where(['task_status_id' => $task_id])->where('creator_id', $user->id)->paginate(10);
        }
        $status = new BoradlessDashboard();
        $status_code = $status->statusCode();

        return backend_view('newmontrealdashboard.sorted_order', compact('query', 'title_name', 'type', 'status_code'));
    }
    ...
    public function montrealSortedData(Datatables $datatables, Request $request)
    {
        ...
        $query = BoradlessDashboard::where('created_at', '>', $start)->where('created_at', '<', $end)->where('is_custom_route', 0)
            ->where('creator_id', $user->id)->where(['task_status_id' => 133]);
    ...
    public function montrealSortedExcel($date = null)
    {
        ...
        $montreal_data = BoradlessDashboard::where('created_at', '>', $start)->where('created_at', '<', $end)->where('is_custom_route', 0)->whereIn('creator_id', $data)->where(['task_status_id' => 133])->get();
        $montreal_array[] = ['JoeyCo Order #', 'Route Number', 'Joey', 'Customer Address', 'Out For Delivery', 'Sorted Time', 'Actual Arrival @ CX', 'Amazon tracking #', 'Status'];
        foreach ($montreal_data as $montreal) {
            $picked_up_at = '';
            $sorted_at = '';
    ...
    public function getMontrealhub(Request $request)
    {
        ...
        return backend_view('newmontrealdashboard.pickup_hub', compact('query','status_code','title_name', 'type'));
    }
    ...
    public function montrealPickedUpData(Datatables $datatables, Request $request)
    {
        ...
        $query = BoradlessDashboard::where('created_at', '>', $start)->where('created_at', '<', $end)->where('is_custom_route', 0)
            ->where('creator_id', $user->id)->where(['task_status_id' => 121]);
    ...
    public function montrealPickedupExcel($date = null)
    {
        ...
        $montreal_data = BoradlessDashboard::where('created_at', '>', $start)->where('created_at', '<', $end)->where('is_custom_route', 0)->whereIn('creator_id', $data)->where(['task_status_id' => 121])->get();
        $montreal_array[] = ['JoeyCo Order #', 'Route Number', 'Joey', 'Customer Address', 'Out For Delivery', 'Sorted Time', 'Actual Arrival @ CX', 'Amazon tracking #', 'Status'];
        foreach ($montreal_data as $montreal) {
            $picked_up_at = '';
            $sorted_at = '';
    ...
    public function getMontnotscan(Request $request)
    {
        ...
        if ($request->get('datepicker')) {
            ...
            $end_dt->setTimeZone(new DateTimezone('UTC'));
            $end = $end_dt->format('Y-m-d H:i:s');
            $query = BoradlessDashboard::where('is_custom_route', 0)->whereIn('task_status_id', [61, 13])->where('created_at', '>', $start)->where('created_at', '<', $end)->where('creator_id', $user->id)->paginate(10);
            // dd($query);
            $notscan = count(DB::table('boradless_dashboard')->where('task_status_id',61)->where('created_at', '>', $start)->where('created_at', '<', $end)->where('creator_id', $user->id)->get());
            $reattempted = count(DB::table('boradless_dashboard')->where('task_status_id', 13)->where('created_at', '>', $start)->where('created_at', '<', $end)->where('creator_id', $user->id)->get());
        } else {
            $notscan = count(DB::table('boradless_dashboard')->where('task_status_id',61)->where('creator_id', $user->id)->get());
            $reattempted = count(DB::table('boradless_dashboard')->where('task_status_id', 13)->where('creator_id', $user->id)->get());
            $query = BoradlessDashboard::where('is_custom_route', 0)->whereIn('task_status_id', [61, 13])->where('creator_id', $user->id)->paginate(10);
        ...
        $status = new BoradlessDashboard();
        $status_code = $status->statusCode();
        return backend_view('newmontrealdashboard.not_scanned_orders', compact('query', 'title_name', 'type', 'status_code','notscan','reattempted'));
    }
    ...
    public function montrealNotScanData(Datatables $datatables, Request $request)
    {
        ...
        return $datatables->eloquent($query)
            ->setRowId(static function ($record) {
                return $record->id==''?$record->id:'N/A';
            })
            ->editColumn('picked_up_at', static function ($record) {
                if ($record->picked_up_at) {
                    $picked_up_at = new \DateTime($record->picked_up_at, new \DateTimeZone('UTC'));
            ...
            ->editColumn('joey_name', static function ($record) {
                return $record->joey_name ? $record->joey_name . ' (' . $record->joey_id . ')' : 'N/A';
            })
            ->editColumn('route_id', static function ($record) {
                return $record->route_id ? 'R-' . $record->route_id . '-' . $record->ordinal : 'N/A';
            })
            ->editColumn('task_status_id', static function ($record) {
                $current_status = $record->task_status_id;
                if ($record->task_status_id == 17) {
    ...
    public function montrealNotscanExcel($date = null)
    {
        ...
        $montreal_data = BoradlessDashboard::where('created_at', '>', $start)->where('created_at', '<', $end)->where('is_custom_route', 0)->whereIn('task_status_id', [61, 13])
    ...
    public function getMontdelivered(Request $request)
    {
        ...
        if ($request->get('datepicker')) {
            ...
            $end_dt->setTimeZone(new DateTimezone('UTC'));
            $end = $end_dt->format('Y-m-d H:i:s');
            $query = BoradlessDashboard::where('is_custom_route', 0)->whereIn('task_status_id', [17, 113, 114, 116, 117, 118, 132, 138, 139, 144])->where('updated_at', 'like', $request->get('datepicker')."%")->where('creator_id', $user->id)->paginate(10);
        } else {
            $query = BoradlessDashboard::where('is_custom_route', 0)->whereIn('task_status_id', [17, 113, 114, 116, 117, 118, 132, 138, 139, 144])->where('creator_id', $user->id)->paginate(10);
        }
        $status = new BoradlessDashboard();
        $status_code = $status->statusCode();
        return backend_view('newmontrealdashboard.delivered_orders', compact('title_name','query','status_code','title_name', 'type'));
    }
    ...
    public function montrealDeliveredData(Datatables $datatables, Request $request)
    {
        ...
        $query = BoradlessDashboard::where('is_custom_route', 0);
    ...
    public function montrealDeliveredExcel($date = null)
    {
        ...
        $montreal_data = BoradlessDashboard::where('created_at', '>', $start)->where('created_at', '<', $end)->where('is_custom_route', 0)
    ...
    public function getMontreturned(Request $request)
    {
        ...
        if ($request->get('datepicker')) {
            ...
            $end_dt->setTimeZone(new DateTimezone('UTC'));
            $end = $end_dt->format('Y-m-d H:i:s');
            $query = BoradlessDashboard::where('is_custom_route', 0)->whereIn('task_status_id', [101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 131, 135, 136, 143])
->where('updated_at', 'like', $request->get('datepicker')."%")->where('creator_id', $user->id)->paginate(10);
        } else {
            $query = BoradlessDashboard::where('is_custom_route', 0)->whereIn('task_status_id', [101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 131, 135, 136, 143])
->where('creator_id', $user->id)->paginate(10);
        }
        $status = new BoradlessDashboard();
        $status_code = $status->statusCode();
        return backend_view('newmontrealdashboard.returned_orders', compact('title_name', 'type','query','status_code'));
    }
...
// dd('s');
        //User Data to get the logged in details...
        $user = Auth::user();
       $datepicker=$request->_;
       
          
        $today_date = !empty($datepicker) ? $datepicker : date("Y-m-d");
        //        $today_date = date('Y-m-d', strtotime($today_date . ' -1 days'));
        ...
        $query = BoradlessDashboard::where('created_at', 'like', $request->datepicker."%")
    ...
    public function montrealReturnedExcel($date = null)
    {
        ...
        $montreal_data = BoradlessDashboard::where('created_at', '>', $start)->where('created_at', '<', $end)->where('is_custom_route', 0)->whereIn('task_status_id', [101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 131, 135, 136, 143])->whereIn('creator_id', $data)->get();
    ...
    public function montrealNotReturnedExcel($date = null)
    {
        ...
        $montreal_data = BoradlessDashboard::where('created_at', '>', $start)->where('created_at', '<', $end)->where('is_custom_route', 0)->whereNull('hub_return_scan')->whereIn('task_status_id', [101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 131, 135, 136, 143])->whereIn('creator_id', $data)->get();
        $montreal_array[] = ['JoeyCo Order #', 'Route Number', 'Joey', 'Customer Address', 'Out For Delivery', 'Sorted Time', 'Joey Returned Scan', 'Hub Returned Scan', 'Amazon tracking #', 'Status'];
        foreach ($montreal_data as $montreal) {
            $picked_up_at = '';
            $sorted_at = '';
    ...
    public function montrealCustomRouteExcel($date = null)
    {
        ...
        $montreal_data = BoradlessDashboard::where('created_at', '>', $start)->where('created_at', '<', $end)->where('is_custom_route', 1)->whereIn('creator_id', $data)->get();
        $montreal_array[] = ['JoeyCo Order #', 'Route Number', 'Joey', 'Customer Address', 'Out For Delivery', 'Sorted Time', 'Actual Arrival @ CX', 'Amazon tracking #', 'Status'];
        foreach ($montreal_data as $montreal) {
            $picked_up_at = '';
            $sorted_at = '';
    ...
    public function montrealHubRouteEdit(Request $request, $routeId, $hubId)
    {
        ...
        $route = $route->get([
            ...
            'sprint__sprints.creator_id',
            'joey_route_locations.distance',
            'sprint__contacts.name',
            'sprint__contacts.phone',
            'joey_route_locations.route_id',
    ...
    public function flagHistoryModelHtmlRender(Request $request)
    {
        ...
        $html = view(
            'backend.newmontrealdashboard.sub-views.ajax-render-view-edit-hub-route-flag-model',
            compact(
                'joey_flags_history',
                'flagCategories',
                'request_data'
    ...
    public function get_trackingorderdetails($sprintId)
    {
        ...
        $result = Sprint::join('sprint__tasks', 'sprint_id', '=', 'sprint__sprints.id')
            ...
            ->get(
                array(
                    'sprint__tasks.*',
                    'joey_routes.id as route_id', \DB::raw("CONVERT_TZ(joey_routes.date,'UTC','America/Toronto') as route_date"),
                    'locations.address',
                    'locations.suite',
                    'locations.postal_code',
                    'sprint__contacts.name',
                    'sprint__contacts.phone',
                    'sprint__contacts.email',
                    'joeys.first_name as joey_firstname',
                    'joeys.id as joey_id',
                    'joeys.last_name as joey_lastname',
                    'vendors.first_name as merchant_firstname',
                    'vendors.last_name as merchant_lastname',
                    'merchantids.scheduled_duetime'
                    ,
                    'joeys.id as joey_id',
                    'merchantids.tracking_id',
                    'joeys.phone as joey_contact',
                    'joey_route_locations.ordinal as stop_number',
                    'merchantids.merchant_order_num',
    ...
    public function addNote(Request $request)
    {
        ...
        if (isset($route->joey_id) && $route->joey_id != null) {
            ...
            $payload = [
                'notification' => ['title' => $subject, 'body' => $message, 'click_action' => 'trackingnote'],
    ...
    public function onPageNotification(Request $request)
    {
        $title_name = 'Montreal';
        $type = 'delivered';
    ...
    public function OrderNotification()
    {
        ...
        $title_name = 'Montreal';
        if (Request()->path() == 'delay-order') {
            $type = 'Delay Order';
            // $notification_query = BoradlessDashboard::where('creator_id', auth()->user()->id)->where('delivered_at', '>', Carbon::now()->toDateTimeString())->paginate(10);
            $notification_query = BoradlessDashboard::where('creator_id', auth()->user()->id)->where('task_status_id', 255)->paginate(10);
        
        } else {
            $type = 'Processing Order';
            // $notification_query = BoradlessDashboard::where('creator_id', auth()->user()->id)->where('created_at', '<', Carbon::now()->toDateTimeString())->where('delivered_at', '<=', Carbon::now()->toDateTimeString())->paginate(10);
            $notification_query = BoradlessDashboard::where('creator_id', auth()->user()->id)->where('task_status_id',124 )->paginate(10);
   
            
        }

        //   where('joey_id',auth()->user()->id)->
        return backend_view('newmontrealdashboard.delay_order', compact('title_name', 'notification_query', 'type'));
