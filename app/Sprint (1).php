...
public function ctcOrders($sprintIds,$from_date, $to_date)
    ...
    {
        return $this->hasone(Task::class, 'sprint_id', 'id')->where('sprint__tasks.type', 'dropoff');
    }
   ...
   /* public function sprintVendor()
    ...
    {
        // gating current routs tasks ids
        return $this->SprintTaskHistory()->select((DB::raw("MAX(CASE WHEN status_id IN (124) THEN CONVERT_TZ(created_at,'UTC','America/Toronto') ELSE NULL END) as athub")))->where('status_id',124)->orderBy('date','ASC')->limit(2)->first();
    }
    ...
    {
        // gating current routs tasks ids
        return $this->SprintTaskHistory()->select((DB::raw("MAX(CASE WHEN status_id=133 THEN CONVERT_TZ(created_at,'UTC','America/Toronto') ELSE NULL END) as sortertime")))->first();
    }
...
//            ->select((DB::raw('MAX(CASE WHEN status_id IN('.$deliver_status.$return_status.') THEN CONVERT_TZ(created_at,"UTC","America/Toronto") ELSE NULL END) as delivery_time')))->first();
        ...
        return $this->SprintTaskHistory()->select((DB::raw('MAX(CASE WHEN status_id IN(17, 113, 114, 116, 117, 118, 132, 138, 139, 144, 131, 101, 102, 103, 104, 105, 106, 107, 108, 109, 111, 112, 131, 135, 136, 143,141) THEN CONVERT_TZ(created_at,"UTC","America/Toronto") ELSE NULL END) as delivery_time')))->first();
    }
