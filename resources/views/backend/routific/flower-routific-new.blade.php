        ...
        @if ($message = Session::get('success'))
            ...
            <div class="page-title">
                <div class="title_left amazon-text">
                    <h3>Flower Routes<small></small></h3>
                </div>
            </div>
...
for($j=0;$j<count($counts);$j++) {
    ...
    echo "<td><button class='details green-gradient btn' data-route-id=" . $counts[$j]->route_id . " data-joey-id=" . $counts[$j]->joey_id . " title='Details'>D</button>
     <a class=' orange-gradient btn' target='_blank' href='../returnroute/" . $counts[$j]->route_id . "/edit/hub/20' title='Edit Route'>E</a>
            ...
            <div class='modal-content'>
                ...
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    ...
                    <select id="zone" multiple class="form-control chosen-select" name="zone[]" requried style="width: 100%;">
                        <?php 
                        $zones =  RoutingZones::where('hub_id','=',20)->whereNull('deleted_at')->get();
...
function fullmap() {
    ...
    <?php    if(empty($_REQUEST['date'])){
                ...
                $.ajax({
                    ...
                    url : '../../allroute/476674/location/joey?date=<?php echo $date; ?>',
                    type : 'GET',
                    dataType:'json',
                    success : function(data) {
                        console.log(data);
