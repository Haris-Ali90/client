...
<!-- <script src="https://code.jquery.com/jquery-1.12.4.js"></script>
  <script src="https://code.jquery.com/ui/1.12.1/jquery-ui.js"></script> -->
    <div class="right_col" role="main">
        ...
        //         <a class=' orange-gradient btn' target='_blank' href='../route/" . $route->route_id . "/edit/hub/17' title='Edit Route'>E</a> 
        //         <button class='transfer  black-gradient btn' data-route-id=" . $route->route_id . " title='Transfer'>T</button> 
        //         <button type='button' class=' red-gradient btn' data-toggle='modal' data-target='#ex5' onclick='initialize(" . $route->route_id . ")' title='Map of Whole Route'>M</button> 
        ...
        //         <button type='button'  class='delete  red-gradient btn'  data-id='" . $route->route_id . "' title='Delete Route'>R</button>
        //         <button class='reroute  orange-gradient btn' data-re=".$route->route_id."  title='Re Route'>RR</button>
        //         </td>";
        //        echo "</tr>";
        //        $j++;
        //        $i++;
        ...
        for($j=0;$j<count($counts);$j++) {
            ...
            echo "<td><button class='details green-gradient btn' data-route-id=" . $counts[$j]->route_id . " data-joey-id=" . $counts[$j]->joey_id . " title='Details'>D</button>
             ...
             <a class=' orange-gradient btn' target='_blank' href='../routific/route/" . $counts[$j]->route_id . "/history' title='Route History'>RH</a>
             </td>";
            echo "</tr>";
            $i++;
        }
    ...
    <div id="ex1" class="modal" style="display: none">
        <div class='modal-dialog'>
            <div class='modal-content'>
                ...
                <select id="slots" multiple class="form-control chosen-select" name="slots[]"  requried style="width: 100%;">
                    <?php 
                    $slots =  Slots::where('hub_id','=',17)->get();
                    foreach ($slots as $slot) {
                        echo "<option value='" . $slot->id . "'>Slot-" . $slot->id ."</option>";
                    }
        ...
        <div class='modal-content'>
            ...
            <input type="hidden" name="_token" value="{{ csrf_token() }}">
                ...
                <select id="joey_id"  name="joey_id" class="form-control">
                    ...
                    $joeys = Joey::whereNull('deleted_at')->where('is_enabled', '=', 1)->whereNull('email_verify_token')->orderBy('first_name')->get();
        ...
        for (var i = 0; i < data.length; i++) {
            if (data[i]['type'] == "dropoff") {
                ...
                bounds.extend(latlng);
                    ...
                    title:   "R-"+data[i]['route_id']+"\nCR-"+data[i]['sprint_id']+"\n("+data[i]['address']+")",
                    title1: "R-"+data[i]['route_id']+"<br>CR-"+data[i]['sprint_id']+"<br>("+data[i]['address']+")"
                });
            
                
                google.maps.event.addListener(marker, 'click', (function(marker, i) {
                    return function() {
                        infowindow.setContent(marker.title1);
    ...
    $(function() {
        ...
        $(".delete").click(function() {
            ...
            if (x) {
                ...
                $.ajax({
                    type: "GET",
                    url: '../returnroute/' + del_id + '/delete/hub',
                    success: function() {
                        location.reload();
        ...
        $(".reroute").click(function(){
            //href='hub/17/re-route/".$route->route_id."'
            var x = confirm("Are you sure you want to re route?");
            if(x){
                    var element = $(this);
                    var del_id = element.attr("data-re");
                    $.ajax({
                        type: "GET",
                        url: '../hub/17/re-route/'+del_id,
              ...
              for (var i = 0; i < data['data'].length; i++) {
                ...
                //  if (data[i]['type'] == "dropoff") {
                      ...
                      if(mapmarker==1)
                          ...
                          title1:   "R-"+data['data'][i][k]['route_id']+"<br>CR-"+data['data'][i][k]['sprint_id']+"<br>("+data['data'][i][k]['address']+")"
                      });
                      ...
                      var myOptions = {
                          ...
                          mapTypeId: google.maps.MapTypeId.ROADMAP
                                      ...
                                      var marker = new google.maps.Marker({
                                          ...
                                          map: map,
                                          icon: icon_marker,
                                          //"https://assets.joeyco.com/images/marker/marker_red"+(j+1)+".png",
